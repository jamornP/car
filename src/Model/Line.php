<?php //require $_SERVER['DOCUMENT_ROOT']."/car/pages/function.php";?>
<?php
namespace App\Model;
use App\Database\Db;
class Line extends Db {
    ...
    public function SentLineSci($name,$dates,$id) {
        $dataCount = $this->CountStatus1();
        $data="\nขอใช้รถตู้คณะฯ พี่อภิสิทธิ์ =: ".$dataCount[4]."\n---------------------\nรวมทั้งหมด/รถคณะฯ : รถสถาบันฯ =: ".$dataCount[5]."/".$dataCount[4].":".$dataCount[0]."\nLink : http://161.246.23.21/car/index.php?id={$id}\n".$name."\nวันที่ ".$dates  ;
            ini_set('display_errors', 1);
            ini_set('display_startup_errors', 1);
